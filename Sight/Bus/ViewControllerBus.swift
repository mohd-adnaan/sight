...
class ViewControllerBus: UIViewController, AVSpeechSynthesizerDelegate {
    ...
    // MARK - Core ML model
    lazy var objectDetectionModel = { return try? bus() }()
...
extension ViewControllerBus: VideoCaptureDelegate {
...
extension ViewControllerBus{
    ...
    func visionRequestDidComplete(request: VNRequest, error: Error?) {
        ...
        if let predictions = request.results as? [VNRecognizedObjectObservation] {
            let filteredPredictions = predictions.filter { $0.labels.first?.identifier == "BusShelter" && $0.confidence >= self.confidenceThreshold }
                ...
                if self.shelterDetected {
                    ...
                    if self.numBoundingBox == 1 || self.numBoundingBox % 15 == 0 {
                        ...
                        if xPosChange > self.xPosThreshold || distanceChange >= self.distanceThreshold {
                            ...
                            // Play success sound if distance is within 2-3 meters
                            if height >= 0.333 && height <= 0.5 {
                                self.playSuccess()
                            }
...
extension ViewControllerBus: UITableViewDataSource {
...
extension ViewControllerBus: 📏Delegate {
...
//    // MARK: - SetUp Blue Bar
//    func setUpBlueBar() {
//        // Modify the dimensions to make the bar thick and cover the entire vertical length
//        let barWidth: CGFloat = 50 // Adjust the width to make it as thick as you want
//        blueBarView = UIView(frame: CGRect(x: 0, y: 0, width: barWidth, height: videoPreview.bounds.height))
//        blueBarView.backgroundColor = UIColor.blue.withAlphaComponent(0.5) // Make it slightly transparent if needed
//        blueBarView.center = CGPoint(x: videoPreview.bounds.midX, y: videoPreview.bounds.midY)
//        videoPreview.addSubview(blueBarView)
//    }
//    
//    override func viewDidLayoutSubviews() {
//        super.viewDidLayoutSubviews()
//        resizePreviewLayer()
//        // Ensure the bar covers the full height
//        blueBarView.frame.size.height = videoPreview.bounds.height
//    }
//    
//    func resizePreviewLayer() {
...
//    func updateBlueBar(with position: CGFloat) {
//        let maxWidth = videoPreview.bounds.width
//        let barCenterX = max(min(position * maxWidth, maxWidth - blueBarView.bounds.width / 2), blueBarView.bounds.width / 2)
//        blueBarView.center.x = barCenterX
//    }
//}
//
//// MARK: - VideoCaptureDelegate
